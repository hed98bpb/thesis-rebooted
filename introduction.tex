...
The continuous Dijkstra works by simulating a \emph{wave front propagation}
from the start point $s$, where at simulation time $t$ the wavefront contains
all the points that are reacheble with distance $d$.  Every time a vertex edge
is reached a new wavefront starts emitting from there. In 1999 Hersberger and
Suri published a paper\cite{HershbergerS99} in which they presented an $O(n\log
n)$ time algorithm which matched the lower bound of the problem (see Chapter
\ref{chapter:lowerbound}), using the continuous Dijkstra method.
