...
determine weather $l$ crosses the polygon $O$. We start by making each obstacle
list of points $o=p_1,p_2,\dots,p_i$ into a list of line segment
...
that in common that the line segment crosses four end points from the polygon.
So we could say it is not allowed to cross four points in of a polygons line
segments. The problem is that it makes (cases a, c and d) illegal. But
fortunately they all have that in common that they are collinear (they lie on a
...
horizontal graph, meaning that given a bigger $k$ doesn't seem to influence the
