...
\begin{mydef}\textbf{(Sorting:)} \nick{cite cormen} \label{def:sorting} \\
...
values and afterwards make a linear scan to see if $x'_i \leq x'_{i+1}$ since the \peter{it is leq right?}
...
in the plane with obstacles (\todo{Insert ref to problem definition}).
